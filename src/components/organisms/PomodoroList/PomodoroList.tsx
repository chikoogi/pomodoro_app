...
export const PomodoroList: React.FC<PomodoroListProps> = ({ todoId }) => {
  ...
  const { sessions, isLoading, addSession, updateSession, deleteSession } =
    usePomodoroManagement(todoId)

  const handleSubmit = (item: Pick<PomodoroSession, 'duration' | 'title'>) => {
    if (editingSessionId) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <Button onClick={() => setShowForm(true)}>새 세션 추가</Button>
      </div>
      ...
      {showForm && (
        <PomodoroForm
          initialDuration={
            editingSessionId
              ? sessions.find((s: PomodoroSession) => s.id === editingSessionId)
                  ?.duration
              : 5 * 60
          }
        ...
        {sessions.map((session: PomodoroSession) => (
          ...
          >
            ...
            {session.status !== 'COMPLETED' && (
              <PomodoroTimer session={session} onUpdate={updateSession} />
            )}
          </div>
        ))}
      </div>
