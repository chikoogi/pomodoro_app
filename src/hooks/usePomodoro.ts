...
import { PomodoroSession, PomodoroSettings, PomodoroState } from '@/types/pomodoro.types';
...
export const usePomodoro = (todoId: string) => {
  const [settings] = useLocalStorage<PomodoroSettings>('pomodoroSettings', DEFAULT_SETTINGS);
  const [sessions, setSessions] = useLocalStorage<PomodoroSession[]>(`pomodoro_${todoId}`, []);
